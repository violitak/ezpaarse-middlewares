...
module.exports = function () {
  ...
  if (!cache) {
    const err = new Error('failed to connect to mongodb, cache not available for panist');
    err.status = 500;
    return err;
  }

  report.set('general', 'panist-queries', 0);
  report.set('general', 'panist-fails', 0);

  return new Promise(function (resolve, reject) {
    cache.checkIndexes(ttl, function (err) {
      if (err) {
        self.logger.error('panist: failed to ensure indexes' + err);
        return reject(new Error('failed to ensure indexes for the cache of panist'));
      }
  ...
  /**
   * enrich ec with cache or api panist
    ...
    return co(function* () {
      ...
      while (buffer.length >= bufferSize || (finalCallback && buffer.length > 0)) {
        ...
        if (packet.ecs.length === 0 || packet.ids.size === 0) {
          self.logger.silly('Panist: no IDs in the paquet');
        ...
        while (!list) {
          if (++tries > maxAttempts) {
            const err = new Error(`Failed to query Panist ${maxAttempts} times in a row`);
          ...
          try {
            ...
            self.logger.error('Panist: ', e.message);
          }
        ...
        for (const item of list) {
          if (item.id) {
            ...
            try {
              ...
              report.inc('general', 'panist-cache-fail');
            }
          }
  ...
  function queryIstex(ids) {
    ...
    return new Promise((resolve, reject) => {
      istex.find(query, { baseUrl: 'https://api.panist.fr' }, (err, result) => {
        if (err) {
          report.inc('general', 'panist-fails');
          return reject(err);
        }
  ...
  /**
   * Enrich ec with api panist and to cache the data in database
   */
