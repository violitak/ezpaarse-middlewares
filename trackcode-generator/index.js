...
module.exports = function anonymizer() {
  ...
  if (!cache) {
    const err = new Error(
      'failed to connect to mongodb, cache not available for trackcode generator'
    );
    err.status = 500;
    return err;
  }
  ...
  function anonymize(ec, next) {
    ...
    return co(function* () {
      if (!ec.host) {
        ec['trackcode'] = crypto.createHmac('sha1', buffer).digest('hex');
        return;
      }
  ...
  return new Promise((resolve, reject) => {
    cache.checkIndexes(ttl, function (err) {
      ...
      crypto.randomBytes(40, function (err, resBuffer) {
