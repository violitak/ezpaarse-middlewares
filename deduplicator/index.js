...
module.exports = function deduplicator() {
  this.logger.verbose('Initializing deduplication');
  ...
  var options = this.job.deduplication = {
    intervals: {
      'misc': 30,
      'html': 10,
      'pdf': 30
  ...
  report.set('dedoublonnage', 'activated', options.use);

  if (options.use) {
    report.set('dedoublonnage', 'strategy', options.strategy);

    for (var letter in options.fieldnames) {
      report.set('dedoublonnage', 'fieldname-' + letter, options.fieldnames[letter]);
    }
    for (var type in options.intervals) {
      report.set('dedoublonnage', 'window-' + type, options.intervals[type]);
    }
  }
  ...
  Window.prototype.push = function (newEC) {
    ...
    if (newEC._meta.strategyField != -1 && newEC.unitid) {
      ...
      for (var j = 0, lgt = this.ecs.length; j < lgt; j++) {
        ...
        var isComparable = (ec._meta.strategyField == strategyField && ec.unitid && ec.rtype);

        if (isComparable) {
          var sameUser      = (ec[strategyField] === newEC[strategyField]);
          var sameRessource = (ec.unitid === newEC.unitid);
          var sameType      = (ec.rtype === newEC.rtype);

          if (sameUser && sameRessource && sameType) {
